...
# Add some new varaibles to the dataset
# extracting the date and the matchup from the matchup variable
...
# creating a variable to determine if the game is in the final 5 minutes
df <- df %>% 
...
# binning the close close_def_dist into 5 bins to simplyfy and group the continous variable
df$close_def_dist_simple <- cut_number(df$close_def_dist,n = 5)

# creating some variables based on teh game by summarising player stats into the one observation
ind_game <- df %>% 
  group_by(team_id,date, Opposition) %>% 
  arrange(team_id,date) %>% 
  summarise(across(c(w),~max(.x, na.rm = TRUE))) %>% 
  ungroup()
# creating varaibles related to win form and total wins
ind_game <- ind_game %>% 
  group_by(team_id) %>% 
...
# bindning the individual game features back to the player dataset
...
# I am not sure if these are errors in the dataset or there is some explanation I can't find. # As they relate to only two players I am inclined to think it is an error in the data
df %>% 
  select(pts,pts_type) %>% 
...
# removing variables that either will have too much of an impact on modelling 
# i.e. if pts is left in it directly correlates to made and missed and the models will only use that data
# the other variables have far too many unique observations to assist with modelling 
df <- df %>% 
...
# I did not want to calculted the FG% using the training data as I thought that would affect
## the out of sample testing
# So i calcaulted the amounts based on the training data, saved that as a variable and 
...
## time as less dummy values are created and therefore less total featires
training <-  training %>% 
  select(-action_type)
...
```{r Boxplot, include=FALSE}
# Thes variables do not seem to have a clear affect on make or miss
training <- training %>% 
  select(-c(Year,
         Overtime))
```
...
# While peforming modelling iterations these varaibles were found to have no affect so were removed
training <- training %>% 
  select(-c(Day,Opposition, Month, shot_clock_simple))
...
mdl_logreg
# The logistic regression model returned the values ROC:0.69 Sens:0.44 Spec:0.82
...
# The decsion tree model found the best cp = 0.001, as the cp incresaes from this point we see a decrease in ROC
...
# due to the significant amount or processing i did most of my iterations on a random sample of 7000 observations from the training data
set.seed(345)

inTrain <- createDataPartition(y = training$shot_result, p = 0.05, list = F)
...
rf_mdl
plot(rf_mdl)
# The random forest model found the best mtry = 16 and using the splitrule = gini
# Values returned were ROC:0.69 Sens:0.48 Spec:0.79
...
# the overall accuracy of the model was 65% which is slightly better than a coin flip
```
...
pROC::roc(response = made_shot_binary,
          ...
          print.thres = 0.5,
          asp = NA)
# due to our lower sens values the AUC is affected, to improve the AUC we would have to devise a way to more accucartly predict made shots
```
