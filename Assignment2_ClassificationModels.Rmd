...
summary(mdl_logreg)
# The logistic regression model returned the values ROC:0.69 Sens:0.44 Spec:0.92
...
## devise a way to more accucartly predict made shots

# Overall I belive the model had difficulty predicting made shots becasue in a real world situation people players will often make lower probability shots (like a long 3pt shot) so the model will predict a miss when it is a make
# But players are less likely to miss a high probability shots (closer than 8ft) so the model will is unlikly to predict a miss when it is a make
# This results that the model will predict significatly more misses and therefore get a higher accucarcy on misses but the trade off is a low accucarcy on makes. 
