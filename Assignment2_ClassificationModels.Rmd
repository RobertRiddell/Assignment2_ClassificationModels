...
df <- df %>% 
  mutate_at(vars(location, shot_result,action_type,shot_type,
                 shot_zone_basic, shot_zone_area, shot_zone_range), as.factor)
...
numeric_vars <- training %>% 
  select(where(is_numeric),where(~length(unique(.x)) > 5) , - c(shot_result, pts_type, shot_zone_basic_FG, 
...
character_vars <- training %>% 
  select(where(is_character),where(~length(unique(.x)) <= 5) ,- c(shot_result, period, action_type, pts_type, FG_2pt_3pt,
...
head(bind_cols(probabilties,class_predictions=class_predictions, observed = training$shot_result))
...
ggplot(data = NULL, aes(probabilties$made, ..count..)) +
  geom_histogram(aes(fill = training$shot_result))

```

```{r ROC Curve}
complete_shot_binary <- if_else(training$shot_result == "made",1,0)

pROC::roc(response = complete_shot_binary,
          predictor = probabilties$made,
          ci = T,
          plot = T,
          legacy.axes = T,
          print.auc = T,
...
```{r Confusion matrix}
# actual predictions
predictions_tree <- predict(tree_mdl, type = "raw")

# probabilties of seeing each result
prob_tree <- predict(tree_mdl,newdata = training ,  type = "prob")
...
predictions_rf <- predict(rf_mdl, type = "raw")

# probabilties of seeing each result
prob_rf <- predict(rf_mdl, type = "prob")
...
```{r ROC Curve, eval=FALSE, include=FALSE}
pROC::roc(repsonse = complete_shot_binary,
          predictor = prob_rf,
          ci = T, 
          plot = T,
          legacy.axes = T,
...
knn_mdl  <- train(shot_result ~. , data = training,
                 method = 'knn',
                 trControl = control_obj,
...
```{r Confusion matrix}
predictions_knn <- predict(knn_mdl, type = "raw")

# probabilties of seeing each result
prob_knn <- predict(knn_mdl, type = "prob")
...
confusionMatrix(data = predictions_knn,
                reference = training$shot_result,
                positive = "made")
...
nb_mdl  <- train(shot_result ~. , data = training,
                 method = 'nb',
                 trControl = control_obj)
...
```{r Confusion matrix}
predictions_nb <- predict(nb_mdl, type = "raw")

# probabilties of seeing each result
prob_nb <- predict(nb_mdl, type = "prob")
...
confusionMatrix(data = predictions_nb,
                reference = training$shot_result,
                positive = "made")
